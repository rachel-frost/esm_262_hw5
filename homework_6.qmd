...
```{r}
#test function "fatal_cost" in the background
test_file(here("tests/test_fatal_cost.R"))
```
...
print(paste("Estimate of average flood events per year:", event_count))
...
```{r}
#test function "bridge_repair" in the background
test_file(here("tests/test_bridge_repair_cost.R"))
```
...
### Test Functions
```{r}
#test functions "test_labor_cost" and "test_material_costs"
test_active_file("tests/test_labor_cost.R")
test_file("tests/test_material_cost.R")
```
...
## Appendix
For help with functions used in this program:
```{r}
...
#insert any function name below for its Help Page:
help(fatal_cost) 
```
